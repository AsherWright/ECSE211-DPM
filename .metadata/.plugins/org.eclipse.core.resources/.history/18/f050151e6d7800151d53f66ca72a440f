...
public class USLocalizer {
	...
	private static int WALL_DIST = 50;
	private static int WALL_GAP = 5;
	...
	public void doLocalization() {
		...
		if (locType == LocalizationType.FALLING_EDGE) {
			...
			tempAngle = odo.getAng(); 
			//Sound.beep();
			//System.out.println(getFilteredData());
			// keep rotating until the robot sees a wall, then latch the angle
			while(getFilteredData() > WALL_DIST && odo.getAng() - tempAngle < 110){
				leftMotor.forward();
				rightMotor.backward();
			}
			//Sound.beep();
			...
			leftMotor.rotate(convertAngle(Lab5.WHEEL_RADIUS, Lab5.TRACK, ZeroPoint), true);
			rightMotor.rotate(-convertAngle(Lab5.WHEEL_RADIUS, Lab5.TRACK, ZeroPoint), false);
			...
			double averageAngle = (angleA + angleB)/2;
			double ZeroPoint =  angleB - averageAngle + 45 - 90;
